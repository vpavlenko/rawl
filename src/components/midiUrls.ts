...
export const processMidiUrlsInApp = (
  ...
  const [_, slug] = location.pathname.split("/f/");
  if (slug) {
    const playSlug = async () => {
      ...
      const filteredMidis = index
        .data()
        .midis.filter(({ slug }) => slug === slug);
      if (filteredMidis.length > 1) {
        alert(`More than one midi is found for a slug ${slug}`);
      } else if (filteredMidis.length === 0) {
        alert(`No midi is found for a slug ${slug}`);
      } else {
