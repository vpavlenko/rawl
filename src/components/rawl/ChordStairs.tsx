...
const ChordStairs: React.FC<{
  mode: Mode;
  pitchClassesUnderCursor?: Set<PitchClass>;
  pitchClassesAroundCursor?: Set<PitchClass>;
}> = React.memo(
  ({ mode, pitchClassesUnderCursor, pitchClassesAroundCursor }) => {
    ...
    const tonicChordPosition = chords.findIndex((chord) =>
      /^(i|I)$/.test(chord),
    );
        ...
        {rehydratedChords.map(({ name, pitches }, index) => (
          <ChordName
            style={{
              ...
              left: index * (NOTE_WIDTH + HORIZONTAL_GAP),
              outline:
                pitchClassesAroundCursor &&
                pitches.every((pitch) =>
                  pitchClassesAroundCursor.has((pitch % 12) as PitchClass),
                )
                  ? "1px dashed white"
                  : "",
              backgroundColor:
                pitchClassesUnderCursor &&
                pitches.every((pitch) =>
                  pitchClassesUnderCursor.has((pitch % 12) as PitchClass),
                )
                  ? "#333"
                  : "black",
            }}
