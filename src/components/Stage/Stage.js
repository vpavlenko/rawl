...
/**
 * id を持つ複数のアイテムを描画、範囲選択、右クリックメニューなどを実装した Canvas
 */
export default function Stage({
  ...
  function drawItems(ctx) {
    ...
    const displayedItems = items.filter(item => viewRect.intersects(item))
    displayedItems.forEach(item => {
      drawItem(ctx, { ...item, selected: selectedItemIDs.includes(item.id) })
    })
  }
  ...
  function onMouseDown(e) {
    ...
    const clickedItems = items.filter(item => new Rect(item).containsPoint(startPos))
