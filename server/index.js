...
console.log(
  "Found %s entries in %s.",
  Object.entries(directories).length,
  DIRECTORIES_PATH,
);
...
console.log(
  "Added %s items (%s tokens) to search trie in %s ms.",
  files.length,
  trie.size,
  time,
);
...
const routes = {
  ...
  metadata: async (params) => {
    ...
    if (params.path) {
      ...
      if ([".mid", ".midi"].includes(ext.toLowerCase())) {
        ...
        const data = await new Promise((resolve, reject) => {
          const stream = fs.createReadStream(
            path.join(LOCAL_CATALOG_ROOT, params.path),
            {
              encoding: "UTF-8",
              start: 0,
              end: 256,
        ...
        if (match && match[1]) {
          ...
          const soundfonts = glob.sync(
            `${LOCAL_CATALOG_ROOT}/${dir}/${name}.sf2`,
            { nocase: true },
          );
        ...
        if (soundfont !== null) {
          soundfont = `${PUBLIC_CATALOG_URL}/${path.join(
            dir,
            path.basename(soundfont),
          )}`;
        }
      }
      ...
      const infoFiles = glob.sync(
        `${LOCAL_CATALOG_ROOT}/${dir}/${name}.{text,txt,doc}`,
        { nocase: true },
      );
      ...
      while (segments.length) {
        ...
        if (imageUrl === null) {
          const imageFiles = glob.sync(
            `${LOCAL_CATALOG_ROOT}/${dir}/*.{gif,png,jpg,jpeg}`,
            { nocase: true },
          );
        ...
        if (infoTexts.length === 0) {
          const infoFiles = glob.sync(
            `${LOCAL_CATALOG_ROOT}/${dir}/*.{text,txt,doc}`,
            { nocase: true },
          );
          infoTexts.push(
            ...infoFiles.map((infoFile) => fs.readFileSync(infoFile, "utf8")),
          );
        }
